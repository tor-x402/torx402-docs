---
description: Glossary of terms used in x402tornado documentation
---
...
### Asset
A digital token or cryptocurrency used for payments. In x402tornado, this typically refers to ETH, USDC, USDT, or other ERC-20/SPL tokens.
...
### Blockchain
A distributed ledger technology that records transactions in blocks linked together cryptographically. x402tornado operates on multiple blockchains including Ethereum-compatible chains and Solana.

## C

### Commitment
A cryptographic value that binds a user to their secret data without revealing it. In x402tornado, the commitment is `C = H1(k||r)` where `k` is the nullifier and `r` is randomness.

### Circom
A circuit compiler for writing arithmetic circuits used in zero-knowledge proofs. x402tornado's zk-SNARK circuits are written in Circom.

### Circuit
In zero-knowledge proofs, a mathematical representation of the computation being proven. x402tornado's circuit proves knowledge of deposit secrets.
...
### Deposit
The act of placing funds into a privacy pool with a secret commitment. The first phase of using x402tornado.
...
### Double-Spending
Attempting to spend the same deposit twice. x402tornado prevents this using nullifier hashes.

## E

### EIP-3009
Ethereum Improvement Proposal 3009, which defines a standard for gasless token transfers using signed authorizations. Used in standard x402 but not in x402tornado (which uses zk-SNARKs instead).
...
The runtime environment for smart contracts on Ethereum and compatible blockchains. x402tornado supports multiple EVM chains.

## F

### Facilitator
A server that provides infrastructure services for x402/x402tornado, including proof verification, blockchain interaction, and pool discovery. Merchants use facilitators to verify and settle payments.
...
### Front-Running
An attack where someone observes a pending transaction and submits their own transaction with a higher gas price to execute first. x402tornado prevents this by binding proofs to specific recipients.

## G

### Groth16
A zero-knowledge SNARK construction that produces small, constant-size proofs with fast verification. Used by x402tornado for privacy proofs.
...
### Hash Function
A cryptographic function that maps arbitrary data to a fixed-size output. x402tornado uses Pedersen Hash (H1) and MiMC (H2).

### HTTP 402 Payment Required
An HTTP status code indicating that payment is required to access a resource. Central to both x402 and x402tornado protocols.

## I

### iden3
An open-source identity and cryptography project that developed circomlib, the library of circuits used by x402tornado.
...
### Merkle Tree
A tree data structure where each non-leaf node is a hash of its children. x402tornado uses Merkle trees to efficiently prove that a commitment exists in the deposit set.

**Properties**: Height 20, supports up to 2^20 (~1M) deposits per pool.

### Merkle Root
The top hash of a Merkle tree, representing the entire tree state. x402tornado stores the last 100 roots to allow flexible proof timing.
...
### Merchant
A server that provides protected HTTP resources and accepts x402tornado payments in exchange for access.

### MiMC Hash
A cryptographic hash function designed for efficient use in zero-knowledge circuits. Used in x402tornado's Merkle tree.
...
### Pedersen Hash
A cryptographically secure hash function based on elliptic curve operations. Used in x402tornado for commitments and nullifier hashing (H1).
...
### Proof (zk-SNARK)
A cryptographic proof that a statement is true without revealing why it's true. In x402tornado, proves deposit ownership without revealing which deposit.

**Structure**: Three elements (pi_a, pi_b, pi_c) in Groth16 format.

### Public Signals
The public inputs to a zero-knowledge proof that are revealed to the verifier. In x402tornado: root, nullifier hash, recipient, relayer, fee.
...
### Scheme
A payment method in x402/x402tornado. Standard x402 supports "exact" (EIP-3009), while x402tornado adds "tornado" (zk-SNARK based).

### Settlement
The final execution of a payment on the blockchain, transferring funds from the privacy pool to the recipient.

### Smart Contract
Self-executing code deployed on a blockchain. x402tornado's privacy pools are implemented as smart contracts.
...
### Sponge Construction
A cryptographic construction for hash functions that processes input data in blocks. MiMC uses sponge construction in x402tornado.

## T

### Tornado Cash
A privacy protocol for Ethereum that pioneered the use of zk-SNARKs for transaction privacy. x402tornado adapts this technology for HTTP micropayments.
...
**Note**: x402tornado plans a multi-party trusted setup ceremony for decentralized trust.

## U

### Unlinkability
The property that two events (e.g., deposit and withdrawal) cannot be connected. Core privacy guarantee of x402tornado.

### USDC
USD Coin, a stablecoin pegged to the US dollar. Commonly used in x402tornado for predictable payment amounts.
...
### Withdrawal
The act of removing funds from a privacy pool by generating a zero-knowledge proof of deposit ownership. The second phase of using x402tornado.

### Witness
In zero-knowledge proofs, the private inputs known only to the prover. In x402tornado: nullifier (k), randomness (r), leaf index (l), Merkle proof.

## X

### x402
An HTTP-native micropayment protocol that uses the 402 status code for payment-required responses. The foundation for x402tornado.
...
### X-PAYMENT Header
An HTTP header containing the payment proof in x402/x402tornado. Base64-encoded JSON with proof data.
...
### zk-SNARK
Zero-Knowledge Succinct Non-interactive ARgument of Knowledge. The specific type of zero-knowledge proof used by x402tornado.
